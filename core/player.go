...
func NewPlayer(conn ziface.IConnection) *Player {
	...
	p := &Player{
		PID:  ID,
		Conn: conn,
		X:    float32(3 + rand.Intn(2)), //随机在160坐标点 基于X轴偏移若干坐标
		Y:    10,                        //高度为
		Z:    float32(3 + rand.Intn(2)), //随机在134坐标点 基于Y轴偏移若干坐标
		V:    1,                         //角度为0，尚未实现
	}
...
//广播玩家位动作
func (p *Player) UpdateAction(actionMsg *pb.PlayerAction) {
	...
	msg := &pb.BroadCast{
		...
		Data: &pb.BroadCast_ActionData{
			ActionData: actionMsg,
		},
	}

	//fmt.Println("ActionApi is called：" + msg.String())
